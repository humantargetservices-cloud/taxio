...
import {
  shouldShowOperatorDashboardPwaCard,
  setPromptDismissed,
...
function renderMobileBottomNav(td, currentTab) {
  const items = [
    ...
    { kind: 'tab', id: 'pricing', label: td.navPrices, icon: icon.euro('h-[18px] w-[18px]') },
  ...
  return `<div id="dash-drawer-root" class="${open ? '' : 'pointer-events-none'}">
    ...
    <aside id="dash-drawer" class="fixed left-0 top-0 z-50 flex h-full w-[min(100%,19rem)] flex-col border-r border-slate-800/80 bg-gradient-to-b from-slate-900 via-slate-900 to-[#0a0f1a] shadow-[4px_0_48px_rgba(0,0,0,0.45)] transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${open ? 'translate-x-0' : '-translate-x-full'}" aria-label="${escapeHtml(td.drawerNavLabel)}">
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-5">
        ...
        <p class="mb-2.5 mt-6 px-3 text-xs font-semibold text-slate-500">${escapeHtml(td.drawerFleet)}</p>
        <div class="space-y-0.5">${fleet.map((item) => drawerNavItem(item.id, item.label, currentTab)).join('')}</div>
...
function renderOverviewBody(td, ctx, tpwa) {
  const { bookingStats, showPwaCard } = ctx
